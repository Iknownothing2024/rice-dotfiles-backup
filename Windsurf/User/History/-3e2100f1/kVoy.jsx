...
const ImageDetail = memo(() => {
  ...
  // Fisher-Yates shuffle algorithm for true randomness - moved outside render
  ...
  const getRelatedImages = (currentImage, galleryImages) => {
    if (!currentImage || !galleryImages || galleryImages.length === 0) {
      return [];
    }
    
    const availableImages = galleryImages.filter(img => img.id !== currentImage.id);
    
    // Return first 3 images from shuffled array
    return shuffleArray(availableImages).slice(0, 3);
  };

  const relatedImages = useMemo(() => getRelatedImages(image, galleryImages), [galleryImages, image.id]);
