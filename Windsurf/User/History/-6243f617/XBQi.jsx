...
import { LifeTracks, IncelLifeTracks } from '../data/musicList.js';
import { useAudioPlayer } from '../hooks/useAudioPlayer';

/** 播放控制组件 */
const PlaybackControls = memo(({ isPlaying, isLoading, onToggle, onPrev, onNext, canSkip }) => (
  <div className="flex items-center gap-2 ml-3" onPointerDown={e => e.stopPropagation()}>
...
const ProgressBar = memo(({ progressBarRef, progressPercentage, onProgressClick, currentTime, duration, formatTime }) => (
  <div className="mb-4 relative z-20" onPointerDown={e => e.stopPropagation()}>
    <div ref={progressBarRef} onClick={onProgressClick} className="relative h-1.5 bg-white/10 rounded-full cursor-pointer group overflow-hidden">
      <div className="absolute top-0 left-0 h-full bg-white/40 rounded-full transition-all duration-300" style={{ width: `${progressPercentage}%` }} />
    </div>
...
const MusicPlayer = memo(({ initialPlaylist = 'life', autoPlay = true }) => {
  ...
  const currentTracks = useMemo(() => playlistType === 'incel' ? IncelLifeTracks : LifeTracks, [playlistType]);
  ...
  return (
    <div ref={constraintsRef} className="fixed inset-0 z-50 pointer-events-none">
      <motion.div drag dragConstraints={constraintsRef} dragMomentum={false} className="absolute bottom-8 right-8 w-80 pointer-events-auto bg-zinc-900/90 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl p-5 select-none touch-none overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-5 relative z-10">
          ...
          <button onClick={handlePlaylistToggle} onPointerDown={e => e.stopPropagation()} className="p-2.5 bg-white/5 hover:bg-white/10 rounded-2xl transition-all">
        ...
        <div className="relative z-10 flex flex-col">
          <ProgressBar
            ...
            duration={duration}
            formatTime={t => `${Math.floor(t/60)}:${Math.floor(t%60).toString().padStart(2,'0')}`}
          />

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-1 group" onPointerDown={e => e.stopPropagation()}>
