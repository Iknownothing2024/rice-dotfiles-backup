  ...
  useEffect(() => {
    const loadEntries = async () => {
      try {
          ...
          markdownFiles.map(async (filename) => {
            try {
              ...
              if (contentType && contentType.includes('text/html')) {
                throw new Error(`Received HTML instead of markdown for ${filename}`);
              }
              
              const text = await response.text();
              
              // Additional check: if response contains HTML tags, it's likely an error
              if (text.includes('<!DOCTYPE') || text.includes('<html')) {
                throw new Error(`Received HTML content instead of markdown for ${filename}`);
              }
        ...
        // If no valid entries, use fallback hardcoded data
          ...
          const fallbackEntries = [
            {
              ...
              date: '2026-02-04',
              author: '人見広介',
              content: 'Crap , I blew too much time on X. I must detox.'
            }
          ];
